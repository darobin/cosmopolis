...
var e = (c9, a8) => {
  for (var b5 in a8)
    d(c9, b5, { get: a8[b5], enumerable: true });
};
...
async function c(e10, r8 = {}) {
  return new Promise((n13, t8) => {
    let o12 = s((i8) => {
      n13(i8), Reflect.deleteProperty(window, `_${a8}`);
    }, true), a8 = s((i8) => {
      t8(i8), Reflect.deleteProperty(window, `_${o12}`);
    }, true);
    window.__TAURI_IPC__({ cmd: e10, callback: o12, error: a8, ...r8 });
  });
}
...
var u3 = class {
  ...
  async listen(e10, a8) {
    return this._handleTauriEvent(e10, a8) ? Promise.resolve(() => {
      let n13 = this.listeners[e10];
      n13.splice(n13.indexOf(a8), 1);
    }) : o(e10, this.label, a8);
  }
  async once(e10, a8) {
    return this._handleTauriEvent(e10, a8) ? Promise.resolve(() => {
      let n13 = this.listeners[e10];
      n13.splice(n13.indexOf(a8), 1);
    }) : u2(e10, this.label, a8);
  }
  async emit(e10, a8) {
    if (P.includes(e10)) {
      for (let n13 of this.listeners[e10] || [])
        n13({ event: e10, id: -1, windowLabel: this.label, payload: a8 });
      return Promise.resolve();
    }
    return m(e10, this.label, a8);
  }
  _handleTauriEvent(e10, a8) {
    return P.includes(e10) ? (e10 in this.listeners ? this.listeners[e10].push(a8) : this.listeners[e10] = [a8], true) : false;
  }
};
var h = class extends u3 {
  ...
  async innerPosition() {
    return a({ __tauriModule: "Window", message: { cmd: "manage", data: { label: this.label, cmd: { type: "innerPosition" } } } }).then(({ x: e10, y: a8 }) => new o2(e10, a8));
  }
  async outerPosition() {
    return a({ __tauriModule: "Window", message: { cmd: "manage", data: { label: this.label, cmd: { type: "outerPosition" } } } }).then(({ x: e10, y: a8 }) => new o2(e10, a8));
  }
  async innerSize() {
    return a({ __tauriModule: "Window", message: { cmd: "manage", data: { label: this.label, cmd: { type: "innerSize" } } } }).then(({ width: e10, height: a8 }) => new l(e10, a8));
  }
  async outerSize() {
    return a({ __tauriModule: "Window", message: { cmd: "manage", data: { label: this.label, cmd: { type: "outerSize" } } } }).then(({ width: e10, height: a8 }) => new l(e10, a8));
  }
  async isFullscreen() {
  ...
  async requestUserAttention(e10) {
    let a8 = null;
    return e10 && (e10 === 1 ? a8 = { type: "Critical" } : a8 = { type: "Informational" }), a({ __tauriModule: "Window", message: { cmd: "manage", data: { label: this.label, cmd: { type: "requestUserAttention", payload: a8 } } } });
  }
  ...
  async onFocusChanged(e10) {
    let a8 = await this.listen("tauri://focus", (d7) => {
      e10({ ...d7, payload: true });
    }), n13 = await this.listen("tauri://blur", (d7) => {
      e10({ ...d7, payload: false });
    });
    return () => {
      a8(), n13();
    };
  ...
  async onFileDropEvent(e10) {
    let a8 = await this.listen("tauri://file-drop", (r8) => {
    ...
    }), d7 = await this.listen("tauri://file-drop-cancelled", (r8) => {
...
var s3 = class extends h {
  constructor(e10, a8 = {}) {
    super(e10), a8 != null && a8.skip || a({ __tauriModule: "Window", message: { cmd: "createWebview", data: { options: { label: e10, ...a8 } } } }).then(async () => this.emit("tauri://created")).catch(async (n13) => this.emit("tauri://error", n13));
  }
  static getByLabel(e10) {
    return M().some((a8) => a8.label === e10) ? new s3(e10, { skip: true }) : null;
  }
};
var b;
...
var __spreadValues = (a8, b5) => {
  for (var prop in b5 || (b5 = {}))
    if (__hasOwnProp2.call(b5, prop))
      __defNormalProp2(a8, prop, b5[prop]);
...
var __spreadProps = (a8, b5) => __defProps(a8, __getOwnPropDescs(b5));
...
var SlTabGroup = class extends ShoelaceElement {
  ...
  connectedCallback() {
    ...
    this.mutationObserver = new MutationObserver((mutations) => {
      if (mutations.some((m7) => !["aria-labelledby", "aria-controls"].includes(m7.attributeName))) {
        setTimeout(() => this.setAriaLabels());
      }
      if (mutations.some((m7) => m7.attributeName === "disabled")) {
...
var SlSplitPanel = class extends ShoelaceElement {
  ...
  handleDrag(event) {
    ...
    drag(this, {
      onMove: (x5, y23) => {
        let newPositionInPixels = this.vertical ? y23 : x5;
        if (this.primary === "end") {
...
var a3 = (t8) => (r8, ...e25) => {
  ...
  let c9, $3 = 0, f8 = false;
  for (; $3 < o12; ) {
    for (c9 = r8[$3]; $3 < o12 && void 0 !== (a24 = e25[$3], i25 = l6(a24)); )
      c9 += i25 + r8[++$3], f8 = true;
    u23.push(a24), n23.push(c9), $3++;
  }
  if ($3 === o12 && n23.push(r8[o12]), f8) {
...
var SlRange = class extends ShoelaceElement {
  ...
  syncTooltip(percent) {
    if (this.output !== null) {
      ...
      if (isRtl) {
        const x5 = `${inputWidth - percentAsWidth}px + ${percent} * ${thumbSize}`;
        this.output.style.translate = `calc((${x5} - ${tooltipWidth / 2}px - ${thumbSize} / 2))`;
      } else {
        const x5 = `${percentAsWidth}px - ${percent} * ${thumbSize}`;
        this.output.style.translate = `calc(${x5} - ${tooltipWidth / 2}px + ${thumbSize} / 2)`;
      }
    }
  }
...
H2.render = function(w6, B) {
  G(w6, B);
};
self.QrCreator = H2;
(function(w6) {
  function B(t8, c9, a8, e34) {
    var b5 = {}, h6 = w6(a8, c9);
    h6.u(t8);
    h6.J();
    e34 = e34 || 0;
    var r8 = h6.h(), d7 = h6.h() + 2 * e34;
    b5.text = t8;
    b5.level = c9;
    b5.version = a8;
    b5.O = d7;
  ...
  function C5(t8, c9, a8, e34, b5, h6, r8, d7, g6, x5) {
    function u9(b23, a24, f8, c24, d24, r23, g23) {
      b23 ? (t8.lineTo(a24 + r23, f8 + g23), t8.arcTo(a24, f8, c24, d24, h6)) : t8.lineTo(a24, f8);
    }
    r8 ? t8.moveTo(c9 + h6, a8) : t8.moveTo(c9, a8);
    u9(d7, e34, a8, e34, b5, -h6, 0);
    u9(g6, e34, b5, c9, b5, 0, -h6);
    u9(x5, c9, b5, c9, a8, h6, 0);
    u9(r8, c9, a8, e34, a8, 0, h6);
  }
  function z5(t8, c9, a8, e34, b5, h6, r8, d7, g6, x5) {
    function u9(b23, a24, c24, d24) {
  ...
  function A4(t8, c9) {
    ...
    if ("string" === typeof a8)
      ...
      var e34 = a8.type, b5 = a8.colorStops;
      a8 = a8.position.map((b23) => Math.round(b23 * c9.size));
      if ("linear-gradient" === e34)
        var h6 = t8.createLinearGradient.apply(t8, a8);
      else if ("radial-gradient" === e34)
        h6 = t8.createRadialGradient.apply(t8, a8);
      else
  ...
  function y23(t8, c9) {
    a: {
      var a8 = c9.text, e34 = c9.v, b5 = c9.N, h6 = c9.K, r8 = c9.P;
      b5 = Math.max(1, b5 || 1);
      for (h6 = Math.min(40, h6 || 40); b5 <= h6; b5 += 1)
        try {
          var d7 = B(a8, e34, b5, r8);
          break a;
        } catch (J) {
        }
      d7 = void 0;
    ...
    if (!d7)
      return null;
    a8 = t8.getContext("2d");
    c9.background && (a8.fillStyle = c9.background, a8.fillRect(c9.left, c9.top, c9.size, c9.size));
    e34 = d7.O;
    h6 = c9.size / e34;
    a8.beginPath();
    for (r8 = 0; r8 < e34; r8 += 1)
      for (b5 = 0; b5 < e34; b5 += 1) {
        var g6 = a8, x5 = c9.left + b5 * h6, u9 = c9.top + r8 * h6, p4 = r8, q = b5, f8 = d7.a, k4 = x5 + h6, m7 = u9 + h6, D3 = p4 - 1, E6 = p4 + 1, n13 = q - 1, l12 = q + 1, y32 = Math.floor(Math.min(0.5, Math.max(0, c9.R)) * h6), v22 = f8(p4, q), I3 = f8(D3, n13), w23 = f8(D3, q);
        D3 = f8(D3, l12);
        var F3 = f8(p4, l12);
        l12 = f8(E6, l12);
        q = f8(
        ...
        k4 = Math.round(k4);
        m7 = Math.round(m7);
        v22 ? C5(g6, x5, u9, k4, m7, y32, !w23 && !p4, !w23 && !F3, !q && !F3, !q && !p4) : z5(g6, x5, u9, k4, m7, y32, w23 && p4 && I3, w23 && F3 && D3, q && F3 && l12, q && p4 && E6);
      }
    A4(a8, c9);
    a8.fill();
    return t8;
  }
  var v5 = { minVersion: 1, maxVersion: 40, ecLevel: "L", left: 0, top: 0, size: 200, fill: "#000", background: null, text: "no text", radius: 0.5, quiet: 0 };
  G = function(t8, c9) {
    var a8 = {};
    Object.assign(a8, v5, t8);
    a8.N = a8.minVersion;
    a8.K = a8.maxVersion;
    a8.v = a8.ecLevel;
    a8.left = a8.left;
    ...
    a8.size = a8.size;
    a8.fill = a8.fill;
    a8.background = a8.background;
    a8.text = a8.text;
    a8.R = a8.radius;
    a8.P = a8.quiet;
    if (c9 instanceof HTMLCanvasElement) {
      if (c9.width !== a8.size || c9.height !== a8.size)
        c9.width = a8.size, c9.height = a8.size;
      c9.getContext("2d").clearRect(0, 0, c9.width, c9.height);
      y23(c9, a8);
    } else
      t8 = document.createElement("canvas"), t8.width = a8.size, t8.height = a8.size, a8 = y23(t8, a8), c9.appendChild(a8);
  };
})(function() {
  function w6(c9) {
    var a8 = C5.s(c9);
    return { S: function() {
      ...
      for (var b5 = 0; b5 < a8.length; b5 += 1)
        c24.put(a8[b5], 8);
    } };
  }
  function B() {
    var c9 = [], a8 = 0, e34 = {
      ...
      c: function(b5) {
        return 1 == (c9[Math.floor(b5 / 8)] >>> 7 - b5 % 8 & 1);
      },
      put: function(b5, h6) {
      ...
      m: function(b5) {
        var h6 = Math.floor(a8 / 8);
        c9.length <= h6 && c9.push(0);
        b5 && (c9[h6] |= 128 >>> a8 % 8);
        a8 += 1;
      }
    };
  ...
  function C5(c9, a8) {
    function e34(b23, h24) {
      for (var a24 = -1; 7 >= a24; a24 += 1)
        if (!(-1 >= b23 + a24 || d7 <= b23 + a24))
          for (var c24 = -1; 7 >= c24; c24 += 1)
            -1 >= h24 + c24 || d7 <= h24 + c24 || (r8[b23 + a24][h24 + c24] = 0 <= a24 && 6 >= a24 && (0 == c24 || 6 == c24) || 0 <= c24 && 6 >= c24 && (0 == a24 || 6 == a24) || 2 <= a24 && 4 >= a24 && 2 <= c24 && 4 >= c24 ? true : false);
    }
    function b5(b23, a24) {
      for (var f8 = d7 = 4 * c9 + 17, k4 = Array(f8), m7 = 0; m7 < f8; m7 += 1) {
        k4[m7] = Array(f8);
        for (var p4 = 0; p4 < f8; p4 += 1)
          k4[m7][p4] = null;
      }
      r8 = k4;
      e34(0, 0);
      ...
      e34(0, d7 - 7);
      f8 = y23.G(c9);
      for (k4 = 0; k4 < f8.length; k4 += 1)
        for (m7 = 0; m7 < f8.length; m7 += 1) {
          p4 = f8[k4];
          var q = f8[m7];
          if (null == r8[p4][q])
            for (var n13 = -2; 2 >= n13; n13 += 1)
              for (var l12 = -2; 2 >= l12; l12 += 1)
                r8[p4 + n13][q + l12] = -2 == n13 || 2 == n13 || -2 == l12 || 2 == l12 || 0 == n13 && 0 == l12;
        }
      for (f8 = 8; f8 < d7 - 8; f8 += 1)
        null == r8[f8][6] && (r8[f8][6] = 0 == f8 % 2);
      for (f8 = 8; f8 < d7 - 8; f8 += 1)
        null == r8[6][f8] && (r8[6][f8] = 0 == f8 % 2);
      f8 = y23.w(h6 << 3 | a24);
      for (k4 = 0; 15 > k4; k4 += 1)
        m7 = !b23 && 1 == (f8 >> k4 & 1), r8[6 > k4 ? k4 : 8 > k4 ? k4 + 1 : d7 - 15 + k4][8] = m7, r8[8][8 > k4 ? d7 - k4 - 1 : 9 > k4 ? 15 - k4 : 14 - k4] = m7;
      r8[d7 - 8][8] = !b23;
      if (7 <= c9) {
        f8 = y23.A(c9);
        for (k4 = 0; 18 > k4; k4 += 1)
          m7 = !b23 && 1 == (f8 >> k4 & 1), r8[Math.floor(k4 / 3)][k4 % 3 + d7 - 8 - 3] = m7;
        for (k4 = 0; 18 > k4; k4 += 1)
          m7 = !b23 && 1 == (f8 >> k4 & 1), r8[k4 % 3 + d7 - 8 - 3][Math.floor(k4 / 3)] = m7;
      }
      if (null == g6) {
        b23 = t8.I(c9, h6);
        f8 = B();
        for (k4 = 0; k4 < x5.length; k4 += 1)
          m7 = x5[k4], f8.put(4, 4), f8.put(m7.b(), y23.f(4, c9)), m7.write(f8);
        for (k4 = m7 = 0; k4 < b23.length; k4 += 1)
          m7 += b23[k4].j;
        if (f8.f() > 8 * m7)
          throw Error("code length overflow. (" + f8.f() + ">" + 8 * m7 + ")");
        for (f8.f() + 4 <= 8 * m7 && f8.put(0, 4); 0 != f8.f() % 8; )
          f8.m(false);
        for (; !(f8.f() >= 8 * m7); ) {
          f8.put(236, 8);
          if (f8.f() >= 8 * m7)
        ...
        for (n13 = 0; n13 < b23.length; n13 += 1) {
          ...
          m7 = Math.max(m7, w23);
          p4[n13] = Array(v22);
          for (l12 = 0; l12 < p4[n13].length; l12 += 1)
            p4[n13][l12] = 255 & f8.B()[l12 + u23];
          u23 += v22;
          l12 = y23.C(w23);
          v22 = z5(p4[n13], l12.b() - 1).l(l12);
          q[n13] = Array(l12.b() - 1);
          for (l12 = 0; l12 < q[n13].length; l12 += 1)
            w23 = l12 + v22.b() - q[n13].length, q[n13][l12] = 0 <= w23 ? v22.c(w23) : 0;
        }
        for (l12 = f8 = 0; l12 < b23.length; l12 += 1)
          f8 += b23[l12].o;
        f8 = Array(f8);
        for (l12 = u23 = 0; l12 < k4; l12 += 1)
          for (n13 = 0; n13 < b23.length; n13 += 1)
            l12 < p4[n13].length && (f8[u23] = p4[n13][l12], u23 += 1);
        for (l12 = 0; l12 < m7; l12 += 1)
          for (n13 = 0; n13 < b23.length; n13 += 1)
            l12 < q[n13].length && (f8[u23] = q[n13][l12], u23 += 1);
        g6 = f8;
      }
      b23 = g6;
      f8 = -1;
      ...
      for (q = d7 - 1; 0 < q; q -= 2)
        for (6 == q && --q; ; ) {
          for (n13 = 0; 2 > n13; n13 += 1)
            null == r8[k4][q - n13] && (l12 = false, p4 < b23.length && (l12 = 1 == (b23[p4] >>> m7 & 1)), a24(k4, q - n13) && (l12 = !l12), r8[k4][q - n13] = l12, --m7, -1 == m7 && (p4 += 1, m7 = 7));
          k4 += f8;
          if (0 > k4 || d7 <= k4) {
            k4 -= f8;
            f8 = -f8;
    ...
    var h6 = A4[a8], r8 = null, d7 = 0, g6 = null, x5 = [], u9 = { u: function(b23) {
      b23 = w6(b23);
      x5.push(b23);
      g6 = null;
    }, a: function(b23, a24) {
      if (0 > b23 || d7 <= b23 || 0 > a24 || d7 <= a24)
        throw Error(b23 + "," + a24);
      return r8[b23][a24];
    }, h: function() {
      return d7;
  ...
  function z5(c9, a8) {
    if ("undefined" == typeof c9.length)
      throw Error(c9.length + "/" + a8);
    var e34 = function() {
      for (var b23 = 0; b23 < c9.length && 0 == c9[b23]; )
        b23 += 1;
      for (var r8 = Array(c9.length - b23 + a8), d7 = 0; d7 < c9.length - b23; d7 += 1)
        r8[d7] = c9[d7 + b23];
      return r8;
    }(), b5 = { c: function(b23) {
      return e34[b23];
      ...
      for (var h6 = Array(b5.b() + a24.b() - 1), c24 = 0; c24 < b5.b(); c24 += 1)
        for (var g6 = 0; g6 < a24.b(); g6 += 1)
          h6[c24 + g6] ^= v5.i(v5.g(b5.c(c24)) + v5.g(a24.c(g6)));
      return z5(h6, 0);
    }, l: function(a24) {
      if (0 > b5.b() - a24.b())
        return b5;
      for (var c24 = v5.g(b5.c(0)) - v5.g(a24.c(0)), h6 = Array(b5.b()), g6 = 0; g6 < b5.b(); g6 += 1)
        h6[g6] = b5.c(g6);
      for (g6 = 0; g6 < a24.b(); g6 += 1)
        h6[g6] ^= v5.i(v5.g(a24.c(g6)) + c24);
      return z5(h6, 0).l(a24);
    } };
    return b5;
  }
  C5.s = function(c9) {
    for (var a8 = [], e34 = 0; e34 < c9.length; e34++) {
      var b5 = c9.charCodeAt(e34);
      128 > b5 ? a8.push(b5) : 2048 > b5 ? a8.push(192 | b5 >> 6, 128 | b5 & 63) : 55296 > b5 || 57344 <= b5 ? a8.push(224 | b5 >> 12, 128 | b5 >> 6 & 63, 128 | b5 & 63) : (e34++, b5 = 65536 + ((b5 & 1023) << 10 | c9.charCodeAt(e34) & 1023), a8.push(240 | b5 >> 18, 128 | b5 >> 12 & 63, 128 | b5 >> 6 & 63, 128 | b5 & 63));
    }
    return a8;
  };
  var A4 = { L: 1, M: 0, Q: 3, H: 2 }, y23 = function() {
    ...
    var a8 = [
      ...
      for (var a24 = b5 << 10; 0 <= c9(a24) - c9(1335); )
        a24 ^= 1335 << c9(a24) - c9(1335);
      return (b5 << 10 | a24) ^ 21522;
    }, A: function(b5) {
      for (var a24 = b5 << 12; 0 <= c9(a24) - c9(7973); )
        a24 ^= 7973 << c9(a24) - c9(7973);
      return b5 << 12 | a24;
    }, G: function(b5) {
      return a8[b5 - 1];
    }, F: function(b5) {
      ...
      for (var a24 = z5([1], 0), c24 = 0; c24 < b5; c24 += 1)
        a24 = a24.multiply(z5([1, v5.i(c24)], 0));
      return a24;
    }, f: function(b5, a24) {
      ...
      for (var a24 = b5.h(), c24 = 0, d7 = 0; d7 < a24; d7 += 1)
        for (var g6 = 0; g6 < a24; g6 += 1) {
          for (var e44 = 0, t22 = b5.a(d7, g6), p4 = -1; 1 >= p4; p4 += 1)
            if (!(0 > d7 + p4 || a24 <= d7 + p4))
              for (var q = -1; 1 >= q; q += 1)
                0 > g6 + q || a24 <= g6 + q || (0 != p4 || 0 != q) && t22 == b5.a(d7 + p4, g6 + q) && (e44 += 1);
          5 < e44 && (c24 += 3 + e44 - 5);
        }
      for (d7 = 0; d7 < a24 - 1; d7 += 1)
        for (g6 = 0; g6 < a24 - 1; g6 += 1)
          if (e44 = 0, b5.a(d7, g6) && (e44 += 1), b5.a(d7 + 1, g6) && (e44 += 1), b5.a(d7, g6 + 1) && (e44 += 1), b5.a(d7 + 1, g6 + 1) && (e44 += 1), 0 == e44 || 4 == e44)
            c24 += 3;
      for (d7 = 0; d7 < a24; d7 += 1)
        for (g6 = 0; g6 < a24 - 6; g6 += 1)
          b5.a(d7, g6) && !b5.a(d7, g6 + 1) && b5.a(d7, g6 + 2) && b5.a(d7, g6 + 3) && b5.a(d7, g6 + 4) && !b5.a(d7, g6 + 5) && b5.a(d7, g6 + 6) && (c24 += 40);
      for (g6 = 0; g6 < a24; g6 += 1)
        for (d7 = 0; d7 < a24 - 6; d7 += 1)
          b5.a(d7, g6) && !b5.a(d7 + 1, g6) && b5.a(d7 + 2, g6) && b5.a(d7 + 3, g6) && b5.a(d7 + 4, g6) && !b5.a(d7 + 5, g6) && b5.a(d7 + 6, g6) && (c24 += 40);
      for (g6 = e44 = 0; g6 < a24; g6 += 1)
        for (d7 = 0; d7 < a24; d7 += 1)
          b5.a(d7, g6) && (e44 += 1);
    ...
    for (var c9 = Array(256), a8 = Array(256), e34 = 0; 8 > e34; e34 += 1)
      c9[e34] = 1 << e34;
    for (e34 = 8; 256 > e34; e34 += 1)
      c9[e34] = c9[e34 - 4] ^ c9[e34 - 5] ^ c9[e34 - 6] ^ c9[e34 - 8];
    ...
    var a8 = [
      ...
      b5 = e44.length / 3;
      a24 = [];
      for (var d7 = 0; d7 < b5; d7 += 1)
        for (var g6 = e44[3 * d7], h6 = e44[3 * d7 + 1], t22 = e44[3 * d7 + 2], p4 = 0; p4 < g6; p4 += 1) {
          var q = t22, f8 = {};
          f8.o = h6;
          f8.j = q;
          a24.push(f8);
...
var SlImageComparer = class extends ShoelaceElement {
  ...
  handleDrag(event) {
    ...
    drag(this.base, {
      onMove: (x5) => {
        this.position = parseFloat(clamp(x5 / width * 100, 0, 100).toFixed(2));
        if (isRtl)
...
function pad2(c9) {
  return c9.length === 1 ? "0" + c9 : String(c9);
}
function rgbToRgb(r8, g6, b5) {
  return {
...
function rgbToHsl(r8, g6, b5) {
  r8 = bound01(r8, 255);
  g6 = bound01(g6, 255);
  b5 = bound01(b5, 255);
  var max = Math.max(r8, g6, b5);
  var min = Math.min(r8, g6, b5);
  var h6 = 0;
  var s12 = 0;
  var l25 = (max + min) / 2;
  if (max === min) {
    ...
    var d7 = max - min;
    s12 = l25 > 0.5 ? d7 / (2 - max - min) : d7 / (max + min);
    switch (max) {
      case r8:
        h6 = (g6 - b5) / d7 + (g6 < b5 ? 6 : 0);
        break;
      case g6:
        h6 = (b5 - r8) / d7 + 2;
        break;
...
function hslToRgb(h6, s12, l25) {
  ...
  return { r: r8 * 255, g: g6 * 255, b: b5 * 255 };
}
function rgbToHsv(r8, g6, b5) {
  r8 = bound01(r8, 255);
  g6 = bound01(g6, 255);
  b5 = bound01(b5, 255);
  var max = Math.max(r8, g6, b5);
  ...
  return { h: h6, s: s12, v: v5 };
}
function hsvToRgb(h6, s12, v5) {
  h6 = bound01(h6, 360) * 6;
  s12 = bound01(s12, 100);
  v5 = bound01(v5, 100);
  ...
  var p4 = v5 * (1 - s12);
  var q = v5 * (1 - f8 * s12);
  var t22 = v5 * (1 - (1 - f8) * s12);
  var mod = i33 % 6;
  var r8 = [v5, q, p4, p4, t22, v5][mod];
  var g6 = [t22, v5, v5, q, p4, p4][mod];
  var b5 = [p4, p4, t22, v5, v5, q][mod];
  return { r: r8 * 255, g: g6 * 255, b: b5 * 255 };
}
function rgbToHex(r8, g6, b5, allow3Char) {
  var hex = [
    pad2(Math.round(r8).toString(16)),
    pad2(Math.round(g6).toString(16)),
...
function rgbaToHex(r8, g6, b5, a8, allow4Char) {
  var hex = [
    pad2(Math.round(r8).toString(16)),
    pad2(Math.round(g6).toString(16)),
    pad2(Math.round(b5).toString(16)),
    pad2(convertDecimalToHex(a8))
  ];
...
function convertDecimalToHex(d7) {
  return Math.round(parseFloat(d7) * 255).toString(16);
}
...
var TinyColor = (
  ...
  function() {
    ...
    TinyColor2.prototype.toHsvString = function() {
      ...
      var v5 = Math.round(hsv.v * 100);
      return this.a === 1 ? "hsv(".concat(h6, ", ").concat(s12, "%, ").concat(v5, "%)") : "hsva(".concat(h6, ", ").concat(s12, "%, ").concat(v5, "%, ").concat(this.roundA, ")");
    };
    ...
    TinyColor2.prototype.toRgbString = function() {
      ...
      var g6 = Math.round(this.g);
      var b5 = Math.round(this.b);
      return this.a === 1 ? "rgb(".concat(r8, ", ").concat(g6, ", ").concat(b5, ")") : "rgba(".concat(r8, ", ").concat(g6, ", ").concat(b5, ", ").concat(this.roundA, ")");
    };
    TinyColor2.prototype.toPercentageRgb = function() {
      var fmt = function(x5) {
        return "".concat(Math.round(bound01(x5, 255) * 100), "%");
      };
      return {
        r: fmt(this.r),
        g: fmt(this.g),
    ...
    TinyColor2.prototype.toPercentageRgbString = function() {
      var rnd = function(x5) {
        return Math.round(bound01(x5, 255) * 100);
      };
    ...
    TinyColor2.prototype.monochromatic = function(results) {
      ...
      while (results--) {
        res.push(new TinyColor2({ h: h6, s: s12, v: v5 }));
...
var SlColorPicker = class extends ShoelaceElement {
  ...
  handleAlphaDrag(event) {
    ...
    drag(container, {
      onMove: (x5) => {
        this.alpha = clamp(x5 / width * 100, 0, 100);
        this.syncValues();
        if (this.value !== oldValue) {
          oldValue = this.value;
          this.emit("sl-change");
  ...
  handleHueDrag(event) {
    ...
    drag(container, {
      onMove: (x5) => {
        this.hue = clamp(x5 / width * 360, 0, 360);
        this.syncValues();
        if (this.value !== oldValue) {
          oldValue = this.value;
          this.emit("sl-change");
  ...
  handleGridDrag(event) {
    ...
    drag(grid, {
      onMove: (x5, y23) => {
        this.saturation = clamp(x5 / width * 100, 0, 100);
...
var m4 = (n32) => ({ name: "arrow", options: n32, async fn(i42) {
  ...
  const T32 = b32 / 2 - R22 / 2, O32 = d32[w32], D3 = P32 - x32[y42] - d32[v32], E32 = P32 / 2 - x32[y42] / 2 + T32, L3 = u6(O32, E32, D3), k22 = null != t5(f32) && E32 != L3 && m32.reference[y42] / 2 - (E32 < O32 ? d32[w32] : d32[v32]) - x32[y42] / 2 < 0;
  return { [h32]: p32[h32] - (k22 ? E32 < O32 ? O32 - E32 : D3 - E32 : 0), data: { [h32]: L3, centerOffset: E32 - L3 } };
} });
...
var b3 = function(e44) {
  return void 0 === e44 && (e44 = {}), { name: "flip", options: e44, async fn(o42) {
    ...
    const O32 = [c32, ...T32], D3 = await s7(o42, b32), E32 = [];
    ...
    if (L3 = [...L3, { placement: r32, overflows: E32 }], !E32.every((t22) => t22 <= 0)) {
      var k22;
      const t22 = ((null == (k22 = a32.flip) ? void 0 : k22.index) || 0) + 1, e52 = O32[t22];
      if (e52)
...
var registry2 = {};
var subscriberQueue = [];
function registerStore(name, store4) {
  if (registry2[name])
    throw new Error(`Store "${name}" already registered.`);
  registry2[name] = store4;
}
...
function safeNotEqual(a8, b5) {
  return a8 != a8 ? b5 == b5 : a8 !== b5 || (a8 && typeof a8 === "object" || typeof a8 === "function");
}

// ui/db/contexts.js
var defaultValue2 = { contexts: await listContexts(), current: await getCurrent() };
var store2 = writable(defaultValue2);
registerStore("contexts", store2);
registerContextListChange((contexts2) => {
  store2.update((data) => ({ ...data, contexts: contexts2 }));
});
registerCurrentChange((current) => {
  store2.update((data) => ({ ...data, current }));
});
...
async function addFeedToContext(feedID, ctxID) {
  if (!ctxID)
    ctxID = store2.current;
  console.warn(`finding in`, store2.contexts);
  const ctx = store2.contexts.find((c9) => c9.$id === ctxID);
  console.warn(`found`, ctx);
  if (!ctx)
    throw new Error(`Can't find context ${ctxID}`);
  if (!ctx.feeds)
    ctx.feeds = [];
...
var store3 = writable(defaultValue3);
registerStore("feed-types", store3);
...
var E5 = (t8, i8) => {
  ...
  let h6, r8 = 2 === i8 ? "<svg>" : "", d7 = v3;
  for (let i9 = 0; i9 < s12; i9++) {
    const s13 = t8[i9];
    let e10, u10, c9 = -1, g6 = 0;
    for (; g6 < s13.length && (d7.lastIndex = g6, u10 = d7.exec(s13), null !== u10); )
      g6 = d7.lastIndex, d7 === v3 ? "!--" === u10[1] ? d7 = a6 : void 0 !== u10[1] ? d7 = f6 : void 0 !== u10[2] ? ($2.test(u10[2]) && (h6 = RegExp("</" + u10[2], "g")), d7 = _3) : void 0 !== u10[3] && (d7 = _3) : d7 === _3 ? ">" === u10[0] ? (d7 = null != h6 ? h6 : v3, c9 = -1) : void 0 === u10[1] ? c9 = -2 : (c9 = d7.lastIndex - u10[2].length, e10 = u10[1], d7 = void 0 === u10[3] ? _3 : '"' === u10[3] ? p3 : m5) : d7 === p3 || d7 === m5 ? d7 = _3 : d7 === a6 || d7 === f6 ? d7 = v3 : (d7 = _3, h6 = void 0);
    const y5 = d7 === _3 && t8[i9 + 1].startsWith("/>") ? " " : "";
    r8 += d7 === v3 ? s13 + l9 : c9 >= 0 ? (n13.push(e10), s13.slice(0, c9) + "$lit$" + s13.slice(c9) + o10 + y5) : s13 + o10 + (-2 === c9 ? (n13.push(void 0), i9) : y5);
  }
  const u9 = r8 + (t8[s12] || "<?>") + (2 === i8 ? "</svg>" : "");
  if (!Array.isArray(t8) || !t8.hasOwnProperty("raw"))
    throw Error("invalid template strings array");
  return [void 0 !== e9 ? e9.createHTML(u9) : u9, n13];
};
var C4 = class {
  constructor({ strings: t8, _$litType$: i8 }, e10) {
    ...
    this.parts = [];
    let h6 = 0, d7 = 0;
    const u9 = t8.length - 1, c9 = this.parts, [v5, a8] = E5(t8, i8);
    if (this.el = C4.createElement(v5, e10), A3.currentNode = this.el.content, 2 === i8) {
      const t9 = this.el.content, i9 = t9.firstChild;
      i9.remove(), t9.append(...i9.childNodes);
    }
    for (; null !== (l12 = A3.nextNode()) && c9.length < u9; ) {
      if (1 === l12.nodeType) {
        if (l12.hasAttributes()) {
          const t9 = [];
          for (const i9 of l12.getAttributeNames())
            if (i9.endsWith("$lit$") || i9.startsWith(o10)) {
              const s12 = a8[d7++];
              if (t9.push(i9), void 0 !== s12) {
                const t10 = l12.getAttribute(s12.toLowerCase() + "$lit$").split(o10), i10 = /([.?@])?(.*)/.exec(s12);
                c9.push({ type: 1, index: h6, name: i10[2], strings: t10, ctor: "." === i10[1] ? M3 : "?" === i10[1] ? k3 : "@" === i10[1] ? H4 : S5 });
              } else
                c9.push({ type: 6, index: h6 });
            }
          for (const i9 of t9)
            l12.removeAttribute(i9);
        }
        if ($2.test(l12.tagName)) {
          const t9 = l12.textContent.split(o10), i9 = t9.length - 1;
          if (i9 > 0) {
            l12.textContent = s10 ? s10.emptyScript : "";
            for (let s12 = 0; s12 < i9; s12++)
              l12.append(t9[s12], r7()), A3.nextNode(), c9.push({ type: 2, index: ++h6 });
            l12.append(t9[i9], r7());
          }
        }
      } else if (8 === l12.nodeType)
        if (l12.data === n11)
          c9.push({ type: 2, index: h6 });
        else {
          let t9 = -1;
          for (; -1 !== (t9 = l12.data.indexOf(o10, t9 + 1)); )
            c9.push({ type: 7, index: h6 }), t9 += o10.length - 1;
        }
      h6++;
    }
  }
...
function P3(t8, i8, s12 = t8, e10) {
  ...
  const u9 = d5(i8) ? void 0 : i8._$litDirective$;
  return (null == r8 ? void 0 : r8.constructor) !== u9 && (null === (n13 = null == r8 ? void 0 : r8._$AO) || void 0 === n13 || n13.call(r8, false), void 0 === u9 ? r8 = void 0 : (r8 = new u9(t8), r8._$AT(t8, s12, e10)), void 0 !== e10 ? (null !== (l12 = (h6 = s12)._$Co) && void 0 !== l12 ? l12 : h6._$Co = [])[e10] = r8 : s12._$Cl = r8), void 0 !== r8 && (i8 = P3(t8, r8._$AS(t8, i8.values), r8, e10)), i8;
}
var V3 = class {
  constructor(t8, i8) {
  ...
  v(t8) {
    ...
    A3.currentNode = o12;
    let n13 = A3.nextNode(), l12 = 0, r8 = 0, d7 = e10[0];
    for (; void 0 !== d7; ) {
      if (l12 === d7.index) {
        let i9;
        2 === d7.type ? i9 = new N2(n13, n13.nextSibling, this, t8) : 1 === d7.type ? i9 = new d7.ctor(n13, d7.name, d7.strings, this, t8) : 6 === d7.type && (i9 = new I2(n13, this, t8)), this.u.push(i9), d7 = e10[++r8];
      }
      l12 !== (null == d7 ? void 0 : d7.index) && (n13 = A3.nextNode(), l12++);
    }
    return o12;
  }
  p(t8) {
...
var N2 = class {
  ...
  k(t8) {
    ...
    let s12, e10 = 0;
    for (const o12 of t8)
      e10 === i8.length ? i8.push(s12 = new N2(this.O(r7()), this.O(r7()), this, this.options)) : s12 = i8[e10], s12._$AI(o12), e10++;
...
var S5 = class {
  ...
  _$AI(t8, i8 = this, s12, e10) {
    ...
    if (void 0 === o12)
      ...
      const e11 = t8;
      let l12, h6;
      for (t8 = o12[0], l12 = 0; l12 < o12.length - 1; l12++)
        h6 = P3(this, e11[s12 + l12], i8, l12), h6 === x3 && (h6 = this._$AH[l12]), n13 || (n13 = !d5(h6) || h6 !== this._$AH[l12]), h6 === b4 ? t8 = b4 : t8 !== b4 && (t8 += (null != h6 ? h6 : "") + o12[l12 + 1]), this._$AH[l12] = h6;
    }
    n13 && !e10 && this.j(t8);
  }
  j(t8) {
...
null == z4 || z4(C4, N2), (null !== (t7 = i7.litHtmlVersions) && void 0 !== t7 ? t7 : i7.litHtmlVersions = []).push("2.6.1");
var Z2 = (t8, i8, s12) => {
  var e10, o12;
  const n13 = null !== (e10 = null == s12 ? void 0 : s12.renderBefore) && void 0 !== e10 ? e10 : i8;
  let l12 = n13._$litPart$;
  if (void 0 === l12) {
    const t9 = null !== (o12 = null == s12 ? void 0 : s12.renderBefore) && void 0 !== o12 ? o12 : null;
    n13._$litPart$ = l12 = new N2(i8.insertBefore(r7(), t9), t9, void 0, null != s12 ? s12 : {});
  }
  return l12._$AI(t8), l12;
};
...
var CosmoContexts = class extends s11 {
  constructor() {
    super();
    const store4 = getStore("contexts");
    this.open = false;
    this.contexts = null;
    this.current = null;
    this.cantEdit = false;
    ...
    this.editing = false;
    this.showHint = false;
    store4.subscribe(({ contexts: contexts2, current }) => {
      ...
      <div id="root">
        ...
        <sl-drawer placement="start" ?open=${!!this.open} @sl-request-close=${this.handleCloseRequest}>
          ${this.contexts && this.contexts.length ? y4`
              <ul>
                ${this.contexts.map(({ $id, name }) => y4`<li>
                <cm-context-link id=${$id} name=${name} ?editable=${this.editing} @click=${this.editing ? () => {
...
var x4 = {};
e(x4, { BaseDirectory: () => F2, Dir: () => F2, copyFile: () => c8, createDir: () => d6, exists: () => v4, readBinaryFile: () => a7, readDir: () => m6, readTextFile: () => l11, removeDir: () => g5, removeFile: () => O3, renameFile: () => _4, writeBinaryFile: () => f7, writeFile: () => u8, writeTextFile: () => u8 });
var F2 = ((n13) => (n13[n13.Audio = 1] = "Audio", n13[n13.Cache = 2] = "Cache", n13[n13.Config = 3] = "Config", n13[n13.Data = 4] = "Data", n13[n13.LocalData = 5] = "LocalData", n13[n13.Desktop = 6] = "Desktop", n13[n13.Document = 7] = "Document", n13[n13.Download = 8] = "Download", n13[n13.Executable = 9] = "Executable", n13[n13.Font = 10] = "Font", n13[n13.Home = 11] = "Home", n13[n13.Picture = 12] = "Picture", n13[n13.Public = 13] = "Public", n13[n13.Runtime = 14] = "Runtime", n13[n13.Template = 15] = "Template", n13[n13.Video = 16] = "Video", n13[n13.Resource = 17] = "Resource", n13[n13.App = 18] = "App", n13[n13.Log = 19] = "Log", n13[n13.Temp = 20] = "Temp", n13[n13.AppConfig = 21] = "AppConfig", n13[n13.AppData = 22] = "AppData", n13[n13.AppLocalData = 23] = "AppLocalData", n13[n13.AppCache = 24] = "AppCache", n13[n13.AppLog = 25] = "AppLog", n13))(F2 || {});
async function l11(i8, t8 = {}) {
  return a({ __tauriModule: "Fs", message: { cmd: "readTextFile", path: i8, options: t8 } });
}
async function a7(i8, t8 = {}) {
  let s12 = await a({ __tauriModule: "Fs", message: { cmd: "readFile", path: i8, options: t8 } });
  return Uint8Array.from(s12);
}
async function u8(i8, t8, s12) {
  typeof s12 == "object" && Object.freeze(s12), typeof i8 == "object" && Object.freeze(i8);
  let e10 = { path: "", contents: "" }, r8 = s12;
  return typeof i8 == "string" ? e10.path = i8 : (e10.path = i8.path, e10.contents = i8.contents), typeof t8 == "string" ? e10.contents = t8 ?? "" : r8 = t8, a({ __tauriModule: "Fs", message: { cmd: "writeFile", path: e10.path, contents: Array.from(new TextEncoder().encode(e10.contents)), options: r8 } });
}
async function f7(i8, t8, s12) {
  typeof s12 == "object" && Object.freeze(s12), typeof i8 == "object" && Object.freeze(i8);
  let e10 = { path: "", contents: [] }, r8 = s12;
  return typeof i8 == "string" ? e10.path = i8 : (e10.path = i8.path, e10.contents = i8.contents), t8 && "dir" in t8 ? r8 = t8 : typeof i8 == "string" && (e10.contents = t8 ?? []), a({ __tauriModule: "Fs", message: { cmd: "writeFile", path: e10.path, contents: Array.from(e10.contents instanceof ArrayBuffer ? new Uint8Array(e10.contents) : e10.contents), options: r8 } });
}
async function m6(i8, t8 = {}) {
  return a({ __tauriModule: "Fs", message: { cmd: "readDir", path: i8, options: t8 } });
}
async function d6(i8, t8 = {}) {
  return a({ __tauriModule: "Fs", message: { cmd: "createDir", path: i8, options: t8 } });
}
async function g5(i8, t8 = {}) {
  return a({ __tauriModule: "Fs", message: { cmd: "removeDir", path: i8, options: t8 } });
}
async function c8(i8, t8, s12 = {}) {
  return a({ __tauriModule: "Fs", message: { cmd: "copyFile", source: i8, destination: t8, options: s12 } });
}
async function O3(i8, t8 = {}) {
  return a({ __tauriModule: "Fs", message: { cmd: "removeFile", path: i8, options: t8 } });
}
async function _4(i8, t8, s12 = {}) {
  return a({ __tauriModule: "Fs", message: { cmd: "renameFile", oldPath: i8, newPath: t8, options: s12 } });
}
async function v4(i8, t8 = {}) {
  return a({ __tauriModule: "Fs", message: { cmd: "exists", path: i8, options: t8 } });
}

// node_modules/tauri-plugin-fs-watch-api/dist-js/index.mjs
var w5 = b;
async function unwatch(id3) {
  await c("plugin:fs-watch|unwatch", { id: id3 });
}
async function watch2(paths, options, cb) {
  const opts = {
    recursive: false,
    delayMs: 2e3,
    ...options
  ...
  const id3 = window.crypto.getRandomValues(new Uint32Array(1))[0];
  await c("plugin:fs-watch|watch", {
    id: id3,
    paths: watchPaths,
    options: opts
  });
  const unlisten = await w5.listen(`watcher://debounced-event/${id3}`, (event) => {
    cb(event.payload);
  });
  return () => {
    void unwatch(id3);
...
var FEED_DIR = "ipfs/feeds";
var BASE_DIR = { dir: F2.AppConfig };
await d6(FEED_DIR, { ...BASE_DIR, recursive: true });
var CuratedFeed = class {
  constructor(data = {}) {
    this.id = data.id || smallid_default();
    this.items = data.items || [];
    this.handlers = [];
    this.eventSink = (evt) => this.handlers.forEach((h6) => h6(evt));
  }
  toJSON() {
    return {
      type: "Feed",
  ...
  static async load(id3, doWatch) {
    const feed = new CuratedFeed(await readFeed(id3));
    if (doWatch)
      feed.stopWatching = await watchFeed(id3, feed.eventSink);
    return feed;
  }
};
async function writeFeed(feed) {
  await u8(`${FEED_DIR}/${feed.id}.json`, JSON.stringify(feed, null, 2), BASE_DIR);
}
async function readFeed(id3) {
  return await l11(`${FEED_DIR}/${id3}.json`, BASE_DIR);
}
async function watchFeed(id3, cb) {
  const stopWatching = await watch2(
    `${FEED_DIR}/${id3}`,
    {},
    (evt) => {
      cb(evt);
